...
const canSubmitChangePassword = () => {
    $('input').on('keyup', () => {
        $("button[type='submit']").prop('disabled', !(!$('.error').hasClass('show') &&
...
const checkPassword = (event) => {
    ...
    $.post(`${window.location.pathname}/checkPassword`, { password }, (data, status, xhr) => {
        if (status !== 'success') {
            ...
            if (data !== 'OK') {
                ...
                $(`.${id}.error ul`).html(data.map((problem) => {
