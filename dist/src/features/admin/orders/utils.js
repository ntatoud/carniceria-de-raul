...
export const getOrderFromId = (req, res, id) => {
    ...
    connection.query(getOrderAndProduct, [id], (error, results) => {
        if (error) {
...
export const orderDelete = (res, order) => {
    const connection = databaseConnect();
    const deleteQuery = `DELETE FROM users WHERE userId = ?;`;
    connection.query(deleteQuery, [order], (error) => {
        if (error)
            throw new Error(error.message);
