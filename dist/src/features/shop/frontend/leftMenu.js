...
const loadSwitch = (name, params) => {
    $(`.form-switch input[name='${name}']`).prop('checked', params.includes(`${name}=on`));
};
const turnOtherSwitchOff = (event) => {
    if ($(event.currentTarget).prop('checked')) {
        const switchName = $(event.currentTarget).attr('name');
        const otherSwitch = $(`.form-switch input[name='${switchName === 'price' ? 'name' : 'price'}']`);
...
// Activate new one
$(`.left-menu-categories .left-menu-item[href='${window.location.pathname}']`).addClass('active');
